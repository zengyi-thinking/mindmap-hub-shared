...
export const useMyMindMaps = () => {
  ...
  const handleCreateMindMap = (mindMapData?: {
    title: string;
    description: string;
    tags: string[];
    shared: boolean;
    ...
    // 使用传入的数据或表单数据
    const title = mindMapData?.title || newMindMapName;
    const description = mindMapData?.description || newMindMapDescription;
    const tags = mindMapData?.tags || 
      newMindMapTags.split(',').map(tag => tag.trim()).filter(tag => tag);
    const shared = mindMapData?.shared !== undefined ? 
      mindMapData.shared : privacyOption === 'public';
